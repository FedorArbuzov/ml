...
def show_performance(iters, train_err_rates, val_err_rates, train_losses, val_losses, figsize=(14, 5), title_str=''):
    ...
    ax1.plot(iters, train_err_rates, 'g-', lw=3, label='train err.rate')
    ax1.plot(iters, val_err_rates, 'r-', lw=3, label='val err.rate')
    xlabel('iterations')
    ylabel('err.rate')
    legend(loc='upper left')

    ax2 = gcf().add_subplot(111, sharex=ax1, frameon=False)
    ax2.plot(iters, train_losses, 'g--', label='train loss')
    ax2.plot(iters, val_losses, 'r--', label='val loss')
